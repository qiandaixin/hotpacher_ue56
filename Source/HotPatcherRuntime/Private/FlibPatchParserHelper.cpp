...
)
{
	...
	auto CoolectPlatformExFiles = [](const FPatchVersionDiff& DiffInfo,const auto& Chunk,ETargetPlatform Platform)->TArray<FExternFileInfo>
	{
		...
		TArray<FExternDirectoryInfo> AllConfiguredExternDirectories;
		TArray<FExternFileInfo> AllConfiguredExternFiles;
		{
			for(int32 PlatformIndex=0;PlatformIndex<Chunk.AddExternAssetsToPlatform.Num();++PlatformIndex)
			{
				if(Chunk.AddExternAssetsToPlatform[PlatformIndex].TargetPlatform == Platform
					|| Chunk.AddExternAssetsToPlatform[PlatformIndex].TargetPlatform == ETargetPlatform::None)
				{
					for (const auto& Directory : Chunk.AddExternAssetsToPlatform[PlatformIndex].AddExternDirectoryToPak)
					{
						if (!Directory.DirectoryPath.Path.IsEmpty())
						{
							...
							AllConfiguredExternDirectories.Add(Directory);
						}
					}
					for (const FExternFileInfo& File : Chunk.AddExternAssetsToPlatform[PlatformIndex].AddExternFileToPak)
					{
						...
						AllConfiguredExternFiles.Add(File);
					}
				}
			}
		}

		if(AllConfiguredExternDirectories.Num())
		{
			TArray<FExternFileInfo> ParsedConfiguredExternFiles = UFlibPatchParserHelper::ParserExDirectoryAsExFiles(AllConfiguredExternDirectories, EHashCalculator::MD5, false);
			for (auto& ConfiguredFile : ParsedConfiguredExternFiles)
			{
				ConfiguredFile.Type = EPatchAssetType::NEW;
				AllFiles.AddUnique(ConfiguredFile);
			}
		}

		for (auto ConfiguredFile : AllConfiguredExternFiles)
		{
			ConfiguredFile.Type = EPatchAssetType::NEW;
			AllFiles.AddUnique(ConfiguredFile);
		}
