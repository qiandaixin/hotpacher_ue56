...
FProjectPackageAssetCollection UFlibHotPatcherCoreHelper::ImportProjectSettingsPackages()
{
	...
	
	{
		...
		const TArray<const ITargetPlatform*> NoTargetPlatforms;
		FGameDelegates::Get().GetModifyCookDelegate().Broadcast(NoTargetPlatforms, PackagesToCook, PackagesToNeverCook);
		for(const auto& PackageName:PackagesToCook)
		{
	...
	)
{
    	...
    	
    	{
    		...
    		for(const auto& ExportObj:ObjectsInPackage)
    		{
    			...
    			if (World && bStorageConcurrent)
    			{
					...
					RootContextData.SaveFlags = SaveFlags;
					FObjectPreSaveRootContext RootContext(RootContextData);
					World->PreSaveRoot(RootContext);
					WorldsToPostSaveRoot.Add(World, RootContextData.bCleanupRequired);
				#endif
    				}
    			...
    			for(const auto& Platform:TargetPlatforms)
    			{
					...
					#else
						FObjectSaveContextData ObjectContextData;
						ObjectContextData.SaveFlags = SaveFlags;
						ObjectContextData.TargetPlatform = Platform;
						FObjectPreSaveContext ObjectPreSaveContext(ObjectContextData);
						ExportObj->PreSave(ObjectPreSaveContext);
					#endif
    					}
    					GIsCookerLoadingPackage = false;
    				}
	...
	if (bStorageConcurrent)
	{
		...
		#else
			FObjectSaveContextData PostRootContextData;
			PostRootContextData.bCleanupRequired = WorldIt.Value();
			FObjectPostSaveRootContext PostRootContext(PostRootContextData);
			World->PostSaveRoot(PostRootContext);
		#endif
		}
	}
