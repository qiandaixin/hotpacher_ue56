			...
			for(auto& Chunk:Context.PakChunks)
			{
				...
				if(Context.GetSettingObject()->IsCookPatchAssets() || Context.GetSettingObject()->GetIoStoreSettings().bIoStore)
				{
					...
					if(Context.GetSettingObject()->IsCookPatchAssets() || bSharedShaderLibrary)
					{
						...
						TArray<FAssetDetail> CookAssets = ChunkAssets;
						if(bOnlyCookShaders) // only cook shaders
						{
							...
							CookAssets = AllShaderAssets;
						}

						if(!bOnlyCookShaders)
						{
							auto AppendWPExternalAssetsForMap = [&Context, &CookAssets](const FString& InMapLongPackageName)
							{
								FString RelativeMapPath = InMapLongPackageName;
								RelativeMapPath.RemoveFromStart(TEXT("/Game/"));

								auto AppendFromDependencies = [&CookAssets, &RelativeMapPath](const FAssetDependenciesInfo& InDependencies)
								{
									const FString ExternalActorsPrefix = FString::Printf(TEXT("/Game/__ExternalActors__/%s/"), *RelativeMapPath);
									const FString ExternalObjectsPrefix = FString::Printf(TEXT("/Game/__ExternalObjects__/%s/"), *RelativeMapPath);

									for (const auto& AssetDetail : InDependencies.GetAssetDetails())
									{
										const FString PackagePath = AssetDetail.PackagePath.ToString();
										if (PackagePath.StartsWith(ExternalActorsPrefix) || PackagePath.StartsWith(ExternalObjectsPrefix))
										{
											CookAssets.AddUnique(AssetDetail);
										}
									}
								};

								AppendFromDependencies(Context.VersionDiff.AssetDiffInfo.AddAssetDependInfo);
								AppendFromDependencies(Context.VersionDiff.AssetDiffInfo.ModifyAssetDependInfo);
							};

							for (const auto& Asset : ChunkAssets)
							{
								if (Asset.AssetType.IsEqual(TEXT("World")))
								{
									FSoftObjectPath WorldPath(Asset.PackagePath.ToString());
									AppendWPExternalAssetsForMap(WorldPath.GetLongPackageName());
								}
							}
						}
						...
						EmptySetting.CookAssets = CookAssets;
						...
						for(const auto& WorldPackage:WorldPackages)
						{
							FExternDirectoryInfo DirectoryInfo;
							{
								...
								FString LongPackageName = ObjectPath.GetLongPackageName();
								FString RelativeMapPath = LongPackageName;
								RelativeMapPath.RemoveFromStart(TEXT("/Game/"));

								FString WorldExternalActorsCookedPath = FPaths::Combine(
									StorageCookedDir,
									PlatformName,
									FApp::GetProjectName(),
									TEXT("Content/__ExternalActors__"),
									RelativeMapPath
								);

								FString WorldExternalActorsMountPath = FString::Printf(
									TEXT("../../../%s/Content/__ExternalActors__/%s"),
									FApp::GetProjectName(),
									*RelativeMapPath
								);
								FPaths::NormalizeFilename(WorldExternalActorsMountPath);

								DirectoryInfo.DirectoryPath.Path = WorldExternalActorsCookedPath;
								DirectoryInfo.MountPoint = WorldExternalActorsMountPath;
							}
							...
							FExternDirectoryInfo ExternalObjectsDirectoryInfo;
							{
								FSoftObjectPath ObjectPath(WorldPackage.ToString());
								FString LongPackageName = ObjectPath.GetLongPackageName();
								FString RelativeMapPath = LongPackageName;
								...
								ExternalObjectsDirectoryInfo.DirectoryPath.Path = FPaths::Combine(
									StorageCookedDir,
									PlatformName,
									FApp::GetProjectName(),
									TEXT("Content/__ExternalObjects__"),
									RelativeMapPath
								);
								ExternalObjectsDirectoryInfo.MountPoint = FString::Printf(
									TEXT("../../../%s/Content/__ExternalObjects__/%s"),
									FApp::GetProjectName(),
									*RelativeMapPath
								);
								FPaths::NormalizeFilename(ExternalObjectsDirectoryInfo.MountPoint);
							}
							if(FPaths::DirectoryExists(ExternalObjectsDirectoryInfo.DirectoryPath.Path))
							{
								const TArray<FExternFileInfo>& WPExternalObjects = UFlibPatchParserHelper::ParserExDirectoryAsExFiles(TArray<FExternDirectoryInfo>{ExternalObjectsDirectoryInfo},Context.GetSettingObject()->GetHashCalculator());
								for(const auto& WPAdditionalFile:WPExternalObjects)
								{
									Context.AddExternalFile(PlatformName,Chunk.ChunkName,WPAdditionalFile);
								}
							}
